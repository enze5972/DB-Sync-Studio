...
public class ValidationRepairRepositoryImplTest {
    ...
    public void shouldPersistValidationRunAndDifferences() throws Exception {
        ...
        ValidationRepositoryImpl repository = new ValidationRepositoryImpl(connectionFactory);
        ...
        ValidationDifferenceDO difference = ValidationDifferenceDO.builder()
        ...
        List<ValidationRunDO> recent = repository.findRecentRuns(10);
        ...
        List<ValidationDifferenceDO> differences = repository.findDifferencesByRunId(runId);
    ...
    public void shouldPersistRepairRunAndDetails() throws Exception {
        ...
        DatabaseConnectionFactory connectionFactory = new DatabaseConnectionFactory(tempDatabase);
        RepairRepositoryImpl repository = new RepairRepositoryImpl(connectionFactory);
        repository.initialize();
        ...
        RepairDetailDO detail = RepairDetailDO.builder()
        ...
        List<RepairRunDO> recent = repository.findRecentRuns(10);
        Assert.assertEquals(1, recent.size());
        Assert.assertEquals(Long.valueOf(runId), recent.get(0).getId());
        Assert.assertEquals("PREVIEWED", recent.get(0).getStatus());

        List<RepairDetailDO> details = repository.findDetailsByRunId(runId);
