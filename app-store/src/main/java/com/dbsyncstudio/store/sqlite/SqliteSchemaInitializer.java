...
public final class SqliteSchemaInitializer {
    ...
    private static void ensureMigrationEntry(Connection connection) throws SQLException {
        ...
        try (Statement statement = connection.createStatement()) {
            statement.execute("INSERT INTO schema_migration_entry (migration_version, migration_name, migration_status, applied_at, message) " +
                    "VALUES (" + CURRENT_SCHEMA_VERSION + ", 'V11__schema_baseline', 'APPLIED', " + System.currentTimeMillis() + ", 'SQLite schema initialized')");
        }
    }
    ...
    private static String readSchemaSql() throws SQLException {
        InputStream inputStream = SqliteSchemaInitializer.class.getResourceAsStream(SCHEMA_RESOURCE);
        if (inputStream == null) {
