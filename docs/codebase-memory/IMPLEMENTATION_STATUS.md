...
- 2026-06-23: release workflow 已拆成 `macos-13` x64 和 `macos-14` aarch64 两条 mac 构建，发布阶段分别下载并只上传 `DB.Sync.Studio_*` 安装器；Windows 发布规则也已从 `**/*.exe` 收窄到 `DB.Sync.Studio_*-setup.exe` / `DB.Sync.Studio_*_en-US.msi`，避免把内置 `java.exe`、`javaw.exe`、`keytool.exe` 一起传到 Release
