...
- 2026-06-23: Windows 安装包把 backend 资源实际落到 `resources/backend` 后，桌面壳仍只按 `resolve_resource("backend")` 取根目录，导致 classpath 指到不存在的 `app-core.jar` 并报 `ClassNotFoundException`；当前已让 `backend.rs` 同时兼容 `backend` / `resources/backend`，并把 Tauri `bundle.resources` 改成显式映射 `resources/backend -> backend`，后续需重新打 Windows 包验证安装目录不再多一层 `resources`
- 2026-06-23: Windows 目标机安装后日志里 `backend` / `java.exe` 路径会带 `\\?\\` 或 `\\?\\UNC\\` verbatim 前缀；Java 进程虽然能启动，但 `-cp` 下的 `app-core.jar` / `lib\\*` 仍会因此报 `ClassNotFoundException`。当前已在 `app-shell/src-tauri/src/backend.rs` 对已解析的 backend 根目录做 verbatim 前缀归一化，再继续拼接 `java.exe` 和 classpath；仓库内新增了 drive/UNC 两个单测用例。完整 `cargo test --manifest-path app-shell/src-tauri/Cargo.toml normalize_windows_process_path` 仍会先被现有 Tauri 资源扫描问题阻塞：`resources/backend/runtime/legal/java.security.sasl/LICENSE` 在 custom build 阶段报 `Permission denied (os error 13)`，与本次逻辑修复无关
- 2026-06-23: GitHub `macos-latest` 已切到 `aarch64-apple-darwin`，Tauri 原生会先产出 `DB Sync Studio_0.1.0_aarch64.dmg`；但 `scripts/package.js` 之前又额外手工重打了一个硬编码 `DB Sync Studio_0.1.0_x64.dmg`，导致发布页出现“文件名写 x64、内容实际是 arm64”的错包；当前已改成优先复用 Tauri 已生成的原生 dmg 文件名，仅在兜底场景才按当前架构推导 `aarch64/x64`
- 2026-06-23: release workflow 已拆成 `macos-15-intel` x64 和 `macos-14` aarch64 两条 mac 构建，发布阶段分别下载并只上传 `DB.Sync.Studio_*` 安装器；Windows 发布规则也已从 `**/*.exe` 收窄到 `DB.Sync.Studio_*-setup.exe` / `DB.Sync.Studio_*_en-US.msi`，避免把内置 `java.exe`、`javaw.exe`、`keytool.exe` 一起传到 Release
- 2026-06-23: Tauri 打包前端之前会被 `scripts/package.js` 和 `tauri.conf.json > beforeBuildCommand` 各构建一次；当前已加 `scripts/tauri-before-build.js` 作为统一入口，`package.js` 驱动 Tauri 时会注入 `DB_SYNC_SKIP_TAURI_UI_BUILD=1` 跳过第二次构建，`app-shell/package.json` 的 `tauri:build` 也已去掉额外的 `build-ui` 前缀
...
- 2026-06-23: 通过 Linux 调试产物确认根因是 AppImage 打包时 `linuxdeploy` 会继续扫描内置 JRE 的 `resources/backend/runtime/lib/libnet.so`，但默认找不到同目录下的 `libjvm.so`，导致 `Failed to deploy dependencies for existing files`；`scripts/package.js` 已在 Linux 打包时自动把内置运行时的 `runtime/lib` 与 `runtime/lib/server` 注入 `LD_LIBRARY_PATH`，让 `linuxdeploy` 能解析这组 JRE 依赖，后续需重新跑 release 验证 AppImage 是否恢复
